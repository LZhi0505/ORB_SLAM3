...
//        Verbose::PrintMess("\t ("+std::to_string(keypt.pt.x)+", "+std::to_string(keypt.pt.y)+"), 去畸变后: ("+std::to_string(keyptUn.pt.x)+", "+std::to_string(keyptUn.pt.y)+")", Verbose::VERBOSITY_DEBUG);
//    }
    ...
    // Step 5：双目立体匹配
    // 只有左图特征点 在右图中 找到了最佳匹配点，才会计算其深度，并存储在 mvDepth 中。key:左图特征点索引，value:其深度值
    // mvuRight 存储的是左图特征点 在 右图匹配到的点的u坐标
...
void Frame::ComputeBoW()
{
    ...
    if(mBowVec.empty())
    {
        ...
        mpORBvocabulary->transform(vCurrentDesc,    // 输入：当前描述子vector
...
//        Verbose::PrintMess("right_idx: "+std::to_string(iR)+", y_right: "+std::to_string(kpY)+", r: "+std::to_string(r)+", max_r: "+std::to_string(maxr)+", min_r: "+std::to_string(minr), Verbose::VERBOSITY_DEBUG);
...
//        Verbose::PrintMess("左图特征点横坐标: "+std::to_string(uL)+", 在右图的搜索范围: ("+std::to_string(minU)+", "+std::to_string(maxU)+"), 最佳匹配右目特征点坐标: ("+std::to_string(mvKeysRight[bestIdxR].pt.y)+", "+std::to_string(mvKeysRight[bestIdxR].pt.x)+"), 最佳匹配距离: "+std::to_string(bestDist), Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess("粗匹配个数: "+std::to_string(num_cu)+", 精匹配个数: "+std::to_string(num_jing)+", correlation_thr: "+std::to_string(thDist)+", num_del: "+std::to_string(num_del)+", num: "+std::to_string(num_jing-num_del), Verbose::VERBOSITY_DEBUG);
}
