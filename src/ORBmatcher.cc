...
int ORBmatcher::SearchByBoW(KeyFrame *pKF, Frame &F, vector<MapPoint *> &vpMapPointMatches) {
    ...
    const DBoW2::FeatureVector &vFeatVecKF =
        pKF->mFeatVec; // 特征向量 mFeatVec，记录 node id（距离叶子节点深度为level up对应的node的Id）及其 对应的图像feature的IDs（该节点下所有叶子节点对应的feature的id）
            ...
            for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++) {
                ...
                if (bestDist1 <= TH_LOW) {
                    ...
                    if (static_cast<float>(bestDist1) < mfNNratio * static_cast<float>(bestDist2)) {
                        ...
                        const cv::KeyPoint &kp = (!pKF->mpCamera2) ? pKF->mvKeysUn[realIdxKF] : (realIdxKF >= pKF->NLeft) ? pKF->mvKeysRight[realIdxKF - pKF->NLeft] : pKF->mvKeys[realIdxKF];
                        // Step 4.4：计算匹配点 旋转角度差 所在的直方图
                        if (mbCheckOrientation) {
                            cv::KeyPoint &Fkp = (!pKF->mpCamera2 || F.Nleft == -1) ? F.mvKeys[bestIdxF] : (bestIdxF >= F.Nleft) ? F.mvKeysRight[bestIdxF - F.Nleft] : F.mvKeys[bestIdxF];
                    ...
                    if (bestDist1R <= TH_LOW) {
                        if (static_cast<float>(bestDist1R) < mfNNratio * static_cast<float>(bestDist2R) || true) {
                            ...
                            if (mbCheckOrientation) {
                                cv::KeyPoint &Fkp = (!F.mpCamera2) ? F.mvKeys[bestIdxFR] : (bestIdxFR >= F.Nleft) ? F.mvKeysRight[bestIdxFR - F.Nleft] : F.mvKeys[bestIdxFR];
...
int ORBmatcher::SearchByProjection(KeyFrame *pKF, Sophus::Sim3<float> &Scw, const std::vector<MapPoint *> &vpPoints, const std::vector<KeyFrame *> &vpPointsKFs, std::vector<MapPoint *> &vpMatched,
...
int ORBmatcher::SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2, vector<pair<size_t, size_t>> &vMatchedPairs, const bool bOnlyStereo, const bool bCoarse) {
    ...
    std::cout << "\t\t相机1的光心坐标" << Cw(0) << ", " << Cw(1) << ", " << Cw(2) << ", 在相机2的三维坐标" << C2(0) << ", " << C2(1) << ", " << C2(2) << ", 在相机2的投影坐标" << ep(0) << ", " << ep(1)
              << std::endl;
 ...
 * Step 2 计算当前帧和上一帧的平移向量
 * Step 3 对于上一帧的每一个地图点，通过相机投影模型，得到投影到当前帧的像素坐标
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono) {
    ...
    const bool bForward =
        tlc(2) > CurrentFrame.mb && !bMono; // 非单目 且 前进：z > 基线b，物体在当前帧的图像上变大，因此对于上一帧的特征点，需要在当前帧更高的尺度上搜索，在当前帧的尺寸小的金字塔图像层上搜索
    ...
    for (int i = 0; i < LastFrame.N; i++) {
        ...
        if (pMP) {
            ...
            if (!LastFrame.mvbOutlier[i]) {
                ...
                // 纯视觉双目：th = 7，纯视觉单目、RGB-D、IMU+单目、IMU+双目、IMU+RGBD：th = 15
                ...
                if (bestDist <= TH_HIGH) {
                    ...
                    if (mbCheckOrientation) {
                        ...
                        if (bin == HISTO_LENGTH)
                            bin = 0;
                        //                            std::cout << "上一帧特征点 " << i << " 的坐标: (" << kpLF.pt.x << ", " << kpLF.pt.y << "), 当前帧特征点 " << bestIdx2 << " 的坐标: (" << kpCF.pt.x
                        //                            << ", " << kpCF.pt.y << ")的rot = " << rot << ", bin = " << bin << std::endl;
                        assert(bin >= 0 && bin < HISTO_LENGTH);
    ...
    Verbose::PrintMess("\t\t\t上一帧特征点对应有地图点的个数：" + std::to_string(nmp) + "，其中内点个数：" + std::to_string(nmpvalid) +
                           "，当前帧与上一帧地图点匹配的个数 nmatches = " + to_string(nmatches),
