...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    // KannalaBrandt 鱼眼相机
    // 双目 或 IMU+双目 或 IMU+RGBD，则读取 mpCamera2
...
//        Verbose::PrintMess("mbf: " + std::to_string(mbf) + ", Depth Threshold (Close/Far Points): " + std::to_string(mThDepth), Verbose::VERBOSITY_DEBUG);
    }
    // 读取RGBD
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if(mState == OK)
    {
        ...
        std::ofstream of_frm("/home/liuzhi/下载/ORB_SLAM3/Examples/f_stereo.txt", std::ios::app);
        Sophus::SE3f Twb = mCurrentFrame.GetPose().inverse();
        Eigen::Quaternionf q = Twb.unit_quaternion();
        Eigen::Vector3f twb = Twb.translation();
        of_frm << setprecision(19) << 1e9*(mCurrentFrame.mTimeStamp) << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    }
    total_frame_num++;
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    if (mState == OK)
    {
        ...
        twb = Twb.translation();
        // 正常的数据集保存时以ns为单位
//        of_frm << setprecision(15) << 1e9*(mCurrentFrame.mTimeStamp) << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    }
    else
    {
    ...
    of_frm << setprecision(15) << (mCurrentFrame.mTimeStamp) << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    total_frame_num++;
    std::cout << "\tvalid frame: " << trackOK_frame_num << ", total frame: " << total_frame_num << endl;
    // -----------------------
 ...
 * @param[in] imuMeasurement IMU测量值
 */
...
/**
 * @brief 对上一帧与当前帧 进行IMU预积分
 * 对于一个帧有两种预积分，一种是相对于上一帧，一种是相对于上一个关键帧
 */
void Tracking::PreintegrateIMU()
{
    // Step 1: 获取两帧之间待处理的预积分数据，组成一个集合
    // 上一帧不存在, 说明两帧之间没有IMU数据，不进行预积分，并将IMU预积分状态置为true
    if(!mCurrentFrame.mpPrevFrame)
    {
        Verbose::PrintMess("non prev frame ", Verbose::VERBOSITY_NORMAL);
        // 设置当前帧已做完预积分 mbImuPreintegrated = true;
    ...
    // 没有IMU数据，不进行预积分，并将IMU预积分状态置为true
...
void Tracking::Track()
{
    ...
    if (bStepByStep)    // 如果处于“逐步”模式，即需要逐步执行
    {
    ...
    if(mpLocalMapper->mbBadImu) // 检查局部地图是否设置了坏的IMU标志。如果是，重置当前活动的地图，然后返回。
    {
    ...
    // 从 Atlas 中取出当前active的地图 pCurrentMap
    Map* pCurrentMap = mpAtlas->GetCurrentMap();    // 获取当前活跃地图 pCurrentMap
    // 检查是否存在活跃地图
    if(!pCurrentMap) {
    ...
    if(mState != NO_IMAGES_YET) {
        // 当前时间戳 < 上一帧时间戳，出现错误，清除imu数据，创建新的子地图，返回
        if(mLastFrame.mTimeStamp > mCurrentFrame.mTimeStamp) {
            cerr << "ERROR: Frame with a timestamp older than previous frame detected!" << endl;
            unique_lock<mutex> lock(mMutexImuQueue);
            mlQueueImuData.clear(); // 清空IMU队列
            CreateMapInAtlas();     // 创建新地图
            return;
        }
        // 当前时间戳 > 上一帧时间戳 + 1秒，检测到时间戳跳变，重置地图，返回
        else if(mCurrentFrame.mTimeStamp > mLastFrame.mTimeStamp + 1.0) {
            // cout << mCurrentFrame.mTimeStamp << ", " << mLastFrame.mTimeStamp << endl;
            // cout << "id last: " << mLastFrame.mnId << "    id curr: " << mCurrentFrame.mnId << endl;
            // 根据是否是 IMU 模式,进行 IMU 的补偿
            // 如果系统处于 IMU模式
            if(mpAtlas->isInertial()) {
                // 如果IMU已经初始化，说明时间戳跳跃可能是由于数据丢失或错误引起的，置状态为 丢失，并重置或创建新的地图
                if(mpAtlas->isImuInitialized()) {
                    cout << "Timestamp jump detected. State set to LOST. Reseting IMU integration..." << endl;
                    // IMU完成第3次初始化（在localmapping线程里）
                    if(!pCurrentMap->GetIniertialBA2()) {
                        mpSystem->ResetActiveMap(); // 如果当前子图中imu没有经过BA2，重置active地图，也就是之前的数据不要了
                    } else {
                        CreateMapInAtlas();         // 如果当前子图中imu进行了BA2，重新创建新的子图，保存当前地图
                    }
                }
                // 如果IMU未初始化，说明时间戳跳变可能是在IMU初始化之前发生的。重置当前活动的地图，以防止在未初始化的情况下跟踪不准确的状态，返回
                else {
    ...
    // IMU零偏：IMU传感器测量数据中的固有误差，如加速度计和陀螺仪的零漂。由于这些误差可能会对IMU数据的精度和稳定性产生影响，因此在SLAM系统中，通常会使用关键帧的IMU零偏 来校准和纠正 当前帧的IMU数据，从而提高SLAM系统的性能和稳定性
    ...
    // Step 4：IMU预积分，当为IMU模式 且 未创建地图时进行
    ...
    // Step 5：进行初始化。若初始化成功，则状态 mState = OK
    if(mState == NOT_INITIALIZED) {
        // 根据不同的传感器类型 进行不同的初始化操作
        // 立体、RGB-D、IMU+立体、IMU+RGB-D初始化
        if(mSensor==System::STEREO || mSensor==System::RGBD || mSensor==System::IMU_STEREO || mSensor==System::IMU_RGBD) {
            Verbose::PrintMess("INITIALIZE: Frame " + std::to_string(mCurrentFrame.mnId) + "，双目或RGB-D初始化...", Verbose::VERBOSITY_DEBUG);
            StereoInitialization();     // 双目初始化，只需1帧
        }
        // 单目初始化：初始化的两张图像必须有一定程度的平移，之后的轨迹都以此平移为单位。通常选择让相机进行左右平移进行初始化，初始化时不能纯旋转
        else {
            Verbose::PrintMess("INITIALIZE: Frame " + std::to_string(mCurrentFrame.mnId) + "，单目初始化...", Verbose::VERBOSITY_DEBUG);
            MonocularInitialization();  // 单目初始化，需要至少2帧
//            Verbose::PrintMess("\t\t\t\t初始化后状态 mState = " + std::to_string(mState) + " (-1: SYSTEM_NOT_READY, 0: NO_IMAGES_YET, 1: NOT_INITIALIZED, 2: OK, 3: RECENTLY_LOST, 4: LOST", Verbose::VERBOSITY_DEBUG);
        }
        ...
        if(mState != OK) {
            ...
            mLastFrame = Frame(mCurrentFrame);  // 将 当前帧 置为 上一帧，以便在下一次迭代中继续尝试初始化
            return;
        }
        ...
        if(mpAtlas->GetAllMaps().size() == 1) {
            mnFirstFrameId = mCurrentFrame.mnId;    // 在跟踪过程中，当需要判断是否需要创建新的关键帧时，可以将当前帧的帧号与 mnFirstFrameId 进行比较，以确定是否需要创建新的关键帧。
        }
    }
    // Step 6：系统已成功初始化，跟踪当前帧
    else {
        //----------------开始第一步跟踪，帧匹配---------------------
        bool bOK;  // 当前帧跟踪状态
        ...
        // 初步估计相机的位姿
        // mbOnlyTracking = false 表示SLAM模式（定位 + 地图更新），mbOnlyTracking = true表示纯定位模式
        // tracking 类构造时默认为 false。在 viewer 中有个开关 ActivateLocalizationMode，可以控制是否开启 mbOnlyTracking
        if(!mbOnlyTracking)
        {
            // 处于 SLAM模式，既 跟踪 又 局部建图，有地图更新
            ...
            if(mState == OK)
            {
                ...
                // Step 6.1 检查并更新上一帧被替换的MapPoints。局部建图线程可能会对原有的地图点 MapPoints 进行替换
                ...
                // Step 6.2 运动模型是无效 且 IMU未初始化 或 刚完成重定位，则跟踪参考关键帧；否则恒速模型跟踪
                // 条件1：如果运动模型无效 且 IMU未初始化,说明是刚开始第一帧跟踪，或者已经跟丢了。
                // 条件2：如果当前帧紧跟着在重定位的帧的后面，我们用重定位帧来恢复位姿
                ...
                // 有速度模型 或 IMU已初始化，则先使用 恒速运动模型进行跟踪；如果跟踪失败，再跟踪参考关键帧
                else {
                ...
                // Step 6.3 如果经过跟踪参考关键帧、恒速模型跟踪都失败的话，并满足一定条件就要标记为 RECENTLY_LOST 或 LOST
                if (!bOK) {
                    // 条件1：如果当前帧 距离 上次重定位成功 < 1s (mnFramesToResetIMU 表示经过多少帧后可以重置IMU，一般设置为和帧率相同，对应的时间是1s)
                    // 条件2：IMU模式
                    // 同时满足条件1，2，标记为LOST
                    ...
                    // 条件1：当前地图中关键帧数目较多（ > 10）
                    // 条件2（隐藏条件）：当前帧 距离上次重定位帧 > 1s（说明还比较争气，值的救）或 非IMU模式
                    // 同时满足条件1，2，则将状态标记为 RECENTLY_LOST，表明在短时间内跟丢，后面会结合IMU预测的位姿看看能不能拽回来
...
//                    Verbose::PrintMess("Lost for a short time", Verbose::VERBOSITY_NORMAL);
                    ...
                    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)) {
                        ...
                        // IMU模式 且 当前帧 距离 跟丢帧超过5s，还没有找回（time_recently_lost默认为5s）
                        // 放弃了，将状态 mState = LOST，跟踪失败 bOK = false
                        if (mCurrentFrame.mTimeStamp - mTimeStampLost > time_recently_lost) {
                            mState = LOST;
                            bOK = false;
                            Verbose::PrintMess("\tIMU模式下，当前帧 距离 跟丢帧超过5s，还没有找回，跟踪丢失，Track Lost... mState = " + std::to_string(mState) + ", bOK = " + std::to_string(bOK), Verbose::VERBOSITY_NORMAL);
                        }
                    }
                    // Step 6.5 纯视觉模式则进行重定位。主要是BOW搜索，EPnP求解位姿
                        ...
                        if(!bOK && mCurrentFrame.mTimeStamp - mTimeStampLost > 3.0f) {
                            // 纯视觉模式下重定位失败，状态为LOST
                            mState = LOST;
                            Verbose::PrintMess("\t\tFailed: 重定位一直失败，且跟踪丢失超过 3s，Track Lost... mState = " + std::to_string(mState) + ", bOK = " + std::to_string(bOK), Verbose::VERBOSITY_NORMAL);
                            bOK = false;
                        }
                    }
                }
                // Step 6.6 LOST状态。长时间丢失，则重置活跃地图或创建一个新地图，并清空最后一个关键帧，然后返回
                else if (mState == LOST) {
        ...
        // 前面主要是两两跟踪（恒速模型跟踪上一帧 / 跟踪参考帧）跟踪一帧得到初始位姿，这里搜索局部关键帧后搜集所有 局部地图点，然后将局部 MapPoints 和 当前帧进行投影匹配，得到更多匹配的 MapPoints后进行 Pose优化
        // 局部的地图: 当前帧、当前帧的 MapPoints、当前关键帧与其它关键帧共视关系
#ifdef REGISTER_TIMES
        std::chrono::steady_clock::time_point time_StartLMTrack = std::chrono::steady_clock::now(); // 局部地图跟踪开始时间
#endif
        // 非 纯定位模式
        if(!mbOnlyTracking) {
            int num_landmark = std::count_if(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(), [](MapPoint* pMp){return pMp !=
        ...
        if(bOK || mState == RECENTLY_LOST)
        {
            ...
            for(list<MapPoint*>::iterator lit = mlpTemporalPoints.begin(), lend =  mlpTemporalPoints.end(); lit != lend; lit++) // 对于临时地图点列表中的每个地图点，删除该地图点
            {
                MapPoint* pMP = *lit;
                delete pMP;
            }
        ...
        if(mState == LOST) {
            ...
            // 如果地图中关键帧数 > 10 且 纯视觉模式(没使用IMU) 或 虽是IMU模式但已完成IMU初始化，则在Atlas中保存当前地图，创建新地图，所有跟状态相关的变量全部重置(包括mState = NO_IMAGES_YET，这使得系统会重新初始化)
            Verbose::PrintMess("\t地图中关键帧数 KeyFramesInMap = " + std::to_string(pCurrentMap->KeyFramesInMap()) + " > 10 且 纯视觉模式 或 虽是IMU模式但已完成IMU初始化，保存当前地图，创建新的地图...", Verbose::VERBOSITY_QUIET);
            CreateMapInAtlas();
            Verbose::PrintMess("\t退出当前跟踪", Verbose::VERBOSITY_NORMAL);
            return;
        }
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        Verbose::PrintMess("\t\t\t\t已创建初始化器，且当前帧的特征点数 = " + std::to_string((int)mCurrentFrame.mvKeys.size()) + " > 100，当前帧 与 初始参考帧ID：" + std::to_string(mInitialFrame.mnId) + "进行特征匹配", Verbose::VERBOSITY_DEBUG);
        ...
        Verbose::PrintMess("\t\t\t\t当前帧与初始参考帧 匹配的特征点数 nmatches = " + std::to_string(nmatches) + " >= 100，初始参考帧 " + std::to_string(mInitialFrame.mnId) + " 作为世界坐标系，其变换矩阵为单位矩阵。开始求解当前帧的姿态", Verbose::VERBOSITY_DEBUG);

//        Verbose::PrintMess("\t\t\t\t当前帧与上一帧 匹配的特征点数 nmatches = " + std::to_string(nmatches) + " >= 100，通过H模型或F模型进行单目初始化，得到两帧间相对运动、初始 MapPoints...", Verbose::VERBOSITY_DEBUG);
        ...
        bool mbMonoInitail = mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,  // 输入：初始参考帧的特征点
        ...
        if(mbMonoInitail)
        {
            ...
            Verbose::PrintMess("\t\t\t\t单目初始化，三角化成功！获取到当前相机的位姿、三维空间点，删除无法进行三角化后的匹配点数 nmatches = " + std::to_string(nmatches) + "，开始创建初始地图点", Verbose::VERBOSITY_DEBUG);
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    vector<MapPoint*> vpMapPointMatches;    // 存储 当前帧特征点 匹配到的 关键帧地图点，key：当前帧特征点索引，value：如果匹配上为 与当前帧的特征点匹配的 关键帧特征点对应的 地图点，未匹配上则为 空

    // Step 2：使用 词袋 BoW 加速 当前帧特征点 与 参考帧地图点 之间的匹配，返回 匹配的特征点数
//    Verbose::PrintMess("\t\t计算属于同一词袋节点的当前帧特征点与参考关键帧特征点对应描述子之间的距离，将最佳距离小于阈值的参考关键帧特征点对应的地图点设为 当前帧匹配特征点的地图点，当前帧的ID：" + to_string(mCurrentFrame.mnId) + "，其参考关键帧的相对ID：" + std::to_string(mpReferenceKF->mnId) + "，绝对ID：" + std::to_string(mpReferenceKF->mnFrameId), Verbose::VERBOSITY_DEBUG);
    int nmatches = matcher.SearchByBoW(mpReferenceKF,
                                       mCurrentFrame,
                                       vpMapPointMatches);  // 输出：当前帧特征点 匹配到的 关键帧地图点。
                                                               // key：当前帧特征点索引，value：与当前帧的特征点匹配的 关键帧特征点对应的 地图点
...
//    Verbose::PrintMess("\t\t将上一帧的位姿作为当前帧的初始位姿，使用PnP优化当前帧位姿", Verbose::VERBOSITY_VERBOSE);
    ...
    else
    {
        ...
        {
            Verbose::PrintMess("\t\t剔除位姿优化后是外点的特征点，则当前帧在参考关键帧中匹配到的地图点数 nmatchesMap = " + std::to_string(nmatchesMap) + " < 10，跟踪失败！！", Verbose::VERBOSITY_VERBOSE);
            // -------------liuzhi加----------
...
//    Verbose::PrintMess("\t\t\t\t开始运动模型跟踪", Verbose::VERBOSITY_DEBUG);
    ...
    // Step 1：(1) 使用参考关键帧更新上一帧的位姿。单目仅更新上一帧的位姿；为什么要更新上一帧的位姿，主要是在ORB_SLAM中优化的是参考关键帧的位姿，对于普通帧，虽然在开始设置了位姿，但是没有参与优化，因此在下一次跟踪时，需要用优化后的参考关键帧的位姿更新上一帧的位姿；
    //         (2) 如果是 双目 或 RGB-D 相机 且 纯定位模式时，当上一帧的地图点深度<阈值 或 从近到远的点数<100时，还会根据深度值生成临时地图点，存放于mLastFrame.mvpMapPoints，并且标记为临时地图点 mLastFrame.mlpTemporalPoints 中。一般只会只会用于更新局部地图，只有当前帧为关键帧时，进行关键帧创建才会生成新的全局地图点；
    UpdateLastFrame();

    // Step 2：根据 IMU 或 恒速运动模型 得到当前帧的初始位姿
    // 如果 IMU完成初始化 且 距离重定位很久(已 > 1s)，则不需要重置 IMU，用 IMU 来估计位姿，并直接返回
    if (mpAtlas->isImuInitialized() && (mCurrentFrame.mnId > mnLastRelocFrameId + mnFramesToResetIMU)) {
        Verbose::PrintMess("\t\tIMU完成初始化, 且 当前帧id"+std::to_string(mCurrentFrame.mnId)+" > 上一重定位帧"+std::to_string(mnLastRelocFrameId)+"+"+std::to_string(mnFramesToResetIMU)+", 用IMU来估计位姿", Verbose::VERBOSITY_DEBUG);
    ...
    {
        Verbose::PrintMess("\t\tIMU未完成初始化 或 当前帧id"+std::to_string(mCurrentFrame.mnId)+" <= 上一重定位帧"+std::to_string(mnLastRelocFrameId)+"+"+std::to_string(mnFramesToResetIMU)+", 用上一帧位姿来估计位姿", Verbose::VERBOSITY_DEBUG);
...
//    Verbose::PrintMess("\t\t将上一帧的有效地图点投影到当前帧，在地图点投影像素坐标附近寻找候选特征点，计算上一帧地图点的描述子和当前帧候选特征点的距离，将最佳距离小于阈值的上一帧地图点设为当前帧匹配特征点的地图点。当前帧的ID：" + to_string(mCurrentFrame.mnId) + "，上一帧的ID：" + to_string(mLastFrame.mnId), Verbose::VERBOSITY_DEBUG);
...
//    Verbose::PrintMess("\t\t使用PnP优化当前帧位姿" ,Verbose::VERBOSITY_VERY_VERBOSE);
    ...
    // 纯视觉模式
        ...
        if(nmatchesMap >= 10) {
            Verbose::PrintMess("\t\t纯视觉，剔除位姿优化后是外点的特征点，则当前帧在上一帧匹配到的地图点数 matchesMap = " + std::to_string(nmatchesMap) + " >= 10，匹配成功！", Verbose::VERBOSITY_DEBUG);
            return true;
        } else {
            Verbose::PrintMess("\t\t纯视觉，剔除位姿优化后是外点的特征点，则当前帧在上一帧匹配到的地图点数 nmatchesMap = " + std::to_string(nmatchesMap) + " < 10，匹配失败！！", Verbose::VERBOSITY_VERBOSE);
            return false;
        }
    }
}
...
//    Verbose::PrintMess("\t\t投影匹配后，当前帧中特征点对应有地图点的个数 aux1 = " + std::to_string(aux1) + "，其中特征点被判定为外点的个数 aux2 = " + std::to_string(aux2), Verbose::VERBOSITY_DEBUG);
    ...
    else
    {
        ...
        if(mCurrentFrame.mnId <= mnLastRelocFrameId + mnFramesToResetIMU)
        {
            Verbose::PrintMess("\t\tIMU已初始化, 但当前帧 <= 上一次重定位帧id "+std::to_string(mnLastRelocFrameId)+"+"+std::to_string(mnFramesToResetIMU)+", 仅优化当前帧位姿", Verbose::VERBOSITY_DEBUG);
            Optimizer::PoseOptimization(&mCurrentFrame);
        }
        ...
        else
        {
            ...
            {
                Verbose::PrintMess("\t\tIMU已初始化, 且 当前帧 > 上一次重定位帧id "+std::to_string(mnLastRelocFrameId)+"+"+std::to_string(mnFramesToResetIMU)+", 且 未更新地图，使用上一帧和IMU联合优化位姿", Verbose::VERBOSITY_DEBUG);
                inliers = Optimizer::PoseInertialOptimizationLastFrame(&mCurrentFrame); // , !mpLastKeyFrame->GetMap()->GetIniertialBA1());
            }
            // 地图已更新，关键帧更准确，所以使用上一关键帧的视觉信息 和 IMU信息，联合优化当前帧位姿、速度和IMU零偏
            else
            {
                Verbose::PrintMess("\t\tIMU已初始化, 且 当前帧 > 上一次重定位帧id "+std::to_string(mnLastRelocFrameId)+"+"+std::to_string(mnFramesToResetIMU)+", 且 已更新地图, 使用上一关键帧和IMU联合优化位姿", Verbose::VERBOSITY_DEBUG);
...
//    Verbose::PrintMess("\t\t当前帧的特征点对应有地图点，且其不是外点，且其地图点被观测数 > 0，则表示该特征点与局部地图点匹配成功", Verbose::VERBOSITY_DEBUG);
    ...
    {
        Verbose::PrintMess("\t\t当前帧id "+std::to_string(mCurrentFrame.mnId)+" < 重定位帧id "+std::to_string(mnLastRelocFrameId)+"+"+std::to_string(mMaxFrames)+"，且成功匹配点的个数 = " + std::to_string(mnMatchesInliers) + " < 50，局部地图跟踪失败！！", Verbose::VERBOSITY_DEBUG);
        return false;
    }
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    {
        // 单目+IMU，且 当前帧距离上一关键帧时间戳超过0.25s，则说明需要插入关键帧
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= 0.25)
        {
            Verbose::PrintMess("\t\tIMU+单目, 且 未完成IMU初始化, 且当前帧距离上一关键帧时间戳>=0.25s, 需要插入关键帧", Verbose::VERBOSITY_DEBUG);
            return true;
        }
        // 双目+IMU 或 RGBD+IMU，且 当前帧距离上一关键帧时间戳超过0.25s，则说明需要插入关键帧
        else if ((mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && (mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= 0.25)
        {
            Verbose::PrintMess("\t\tIMU+双目/RGBD, 且 未完成IMU初始化, 且当前帧距离上一关键帧时间戳>=0.25s, 需要插入关键帧", Verbose::VERBOSITY_DEBUG);
            return true;
        }
        else
        {
            Verbose::PrintMess("\t\tIMU模式 且 未完成IMU初始化, 但当前帧距离上一关键帧时间戳<0.25s, 不需要插入关键帧", Verbose::VERBOSITY_DEBUG);
            return false;
        }
    }
    Verbose::PrintMess("\t\t非IMU模式 或 IMU模式已完成IMU初始化, 需进行后续判断", Verbose::VERBOSITY_DEBUG);
    ...
    {
        Verbose::PrintMess("\t\t关键帧个数"+std::to_string(nKFs)+" > "+std::to_string(mMaxFrames)+" 且 当前帧id"+std::to_string(mCurrentFrame.mnId)+" < 上一次重定位id "+std::to_string(mnLastRelocFrameId)+" + "+std::to_string(mMaxFrames)+", 不需要插入关键帧", Verbose::VERBOSITY_DEBUG);
        return false;
    }
    ...
    Verbose::PrintMess("\t\t近地图点是内点的个数: " + to_string(nTrackedClose) + " 需 < 100; 近地图点不存在或是外点的个数: " + to_string(nNonTrackedClose) + " 需 > 70。是否需插入关键帧: "+std::to_string(bNeedToInsertClose), Verbose::VERBOSITY_DEBUG);
    ...
    if(mpCamera2) thRefRatio = 0.75f;   // 双目非立体匹配
    ...
    Verbose::PrintMess("\t\t当前帧跟踪地图点数 " + to_string(mnMatchesInliers) + ", 需 < 参考关键帧地图点阈值 "+std::to_string(nRefMatches)+" * "+std::to_string(thRefRatio)+" = " + to_string(nRefMatches * thRefRatio), Verbose::VERBOSITY_DEBUG);
    ...
    if(((c1a||c1b||c1c) && c2)||c3 ||c4)
    {
        ...
        // Step 7.6：局部建图local mapping空闲时 或 正在做IMU初始化时可以直接插入，不空闲的时候要根据情况插入
        if(bLocalMappingIdle || mpLocalMapper->IsInitializing())
        {
            Verbose::PrintMess("\t\t局部建图空闲 或 正在IMU初始化，需插入关键帧", Verbose::VERBOSITY_DEBUG);
            return true;
        }
        // (隐藏条件) 局部建图在忙 且 未在进行IMU初始化
        else
        {
            mpLocalMapper->InterruptBA();
            // 双目、双目+IMU、RGB-D、RGB-D+IMU时，如队列里没有阻塞太多关键帧，可以插入
            // tracking插入关键帧不是直接插入，而且先插入到 mlNewKeyFrames中，
            ...
            if(mSensor != System::MONOCULAR  && mSensor != System::IMU_MONOCULAR)
            {
                ...
                {
                    Verbose::PrintMess("\t\t双目、双目+IMU、RGB-D、RGB-D+IMU，队列中的关键帧个数 < 3，需插入关键帧", Verbose::VERBOSITY_DEBUG);
                    return true;
                }
                // 队列中缓冲的关键帧数目太多, 暂时不能插入
                else
                {
                    Verbose::PrintMess("\t\t双目、双目+IMU、RGB-D、RGB-D+IMU，队列中的关键帧个数 >= 3，不能插入关键帧", Verbose::VERBOSITY_DEBUG);
                    return false;
                }
            }
            // 单目、单目+IMU
            else
            {
                ...
                //? 为什么这里对单目情况的处理不一样?
                //回答：可能是单目关键帧相对比较密集
                Verbose::PrintMess("\t\t单目、单目+IMU，不需要插入关键帧", Verbose::VERBOSITY_DEBUG);
                return false;
            }
        }
    }
...
//                    std::cout << "j: " << j << ", depth: " << vDepthIdx[j].first << ", newPoints: " << newPoints << ", oldPoints: " << oldPoints << std::endl;
                ...
                {
                    bCreateNew = true;
//                    std::cout << "j: " << j << ", depth: " << vDepthIdx[j].first << ", newPoints: " << newPoints << ", oldPoints: " << oldPoints << ", lm.obs: " << pMP->Observations() << std::endl;
                    mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
                }
...
//                    std::cout << "j: " << j << ", depth: " << vDepthIdx[j].first << ", newPoints: " << newPoints << ", oldPoints: " << oldPoints << ", lm.obs: " << pMP->Observations() << std::endl;
            ...
            Verbose::PrintMess("\t插入新关键帧 id: " + std::to_string(pKF->mnId) + " ("+std::to_string(pKF->mnFrameId)+")，新建了 " + std::to_string(newPoints) + " 个全局地图点", Verbose::VERBOSITY_DEBUG);
        }
    }
...
//        Verbose::PrintMess("\t\tth = "+std::to_string(th)+", mbFarPoints = "+std::to_string(mpLocalMapper->mbFarPoints)+", mThFarPoints = "+std::to_string(mpLocalMapper->mThFarPoints), Verbose::VERBOSITY_DEBUG);
        // 投影匹配得到更多的匹配关系
...
//                continue;
    ...
    Verbose::PrintMess("\t\t局部关键帧的个数: "+std::to_string(mvpLocalKeyFrames.size())+", 当前帧的参考关键帧id: "+std::to_string(mpReferenceKF->mnId)+"("+std::to_string(mpReferenceKF->mnFrameId)+")", Verbose::VERBOSITY_DEBUG);
}
...
bool Tracking::Relocalization()
{
    ...
    for(int i = 0; i < nKFs; i++)
    {
        ...
        else
        {
            ...
            {
                vbDiscarded[i] = true;
                Verbose::PrintMess("\t\t\t\t\t第 " + std::to_string(i) + " 个候选关键帧（相对ID:" + std::to_string(pKF->mnId) + "，绝对ID:" + std::to_string(pKF->mnFrameId) + "）和 当前帧 词袋匹配的特征点数 nmatches = " + std::to_string(nmatches) + " < 15，该关键帧 被放弃", Verbose::VERBOSITY_NORMAL);
                SaveSearchByBowFailed(mCurrentFrame, pKF, vvpMapPointMatches[i], 3);   // 3表示重定位中的词袋匹配时丢失
                continue;
            }
            else
            {
                Verbose::PrintMess("\t\t\t\t\t第 " + std::to_string(i) + " 个候选关键帧（相对ID:" + std::to_string(pKF->mnId) + "，绝对ID:" + std::to_string(pKF->mnFrameId) + "）和 当前帧 匹配的特征点数 nmatches = " + std::to_string(nmatches) + " >= 15，且当前帧匹配成功的特征点都已在该关键帧中匹配到地图点，该关键帧 被保留", Verbose::VERBOSITY_NORMAL);
    ...
    while(nCandidates > 0 && !bMatch)
    {
        ...
        for(int i = 0; i < nKFs; i++)
        {
            ...
            if(bTcw)
            {
                ...
                if(nGood < 50)
                {
                    Verbose::PrintMess("\t\t\t\t\t\t第一次BA优化之后的内点数 10 <= nGood = " + std::to_string(nGood) + " < 50，可通过投影的方式对之前 未匹配的点 再次进行匹配，抢救一下", Verbose::VERBOSITY_NORMAL);
                    ...
                    {
                        Verbose::PrintMess("\t\t\t\t\t\t\t通过投影过程新增的匹配特征点数 nadditional = " + std::to_string(nadditional) + " + 第一次优化之后的内点数 nGood " + std::to_string(nGood) + " = " + std::to_string(nadditional + nGood) + " >= 50，进行第二次BA优化位姿", Verbose::VERBOSITY_NORMAL);
                        ...
                        {
                            Verbose::PrintMess("\t\t\t\t\t\t\t第二次BA优化之后的内点数 30 < nGood =  " + std::to_string(nGood) + " < 50，可再次通过投影的方式对之前 未匹配的点 进行匹配，再抢救一下", Verbose::VERBOSITY_NORMAL);
                            ...
                            {
                                Verbose::PrintMess("\t\t\t\t\t\t\t再次通过投影过程新增的匹配特征点数 nadditional = " + std::to_string(nadditional) + " + 第二次优化之后的内点数 nGood " + std::to_string(nGood) + " = " + std::to_string(nadditional + nGood) + " >= 50，进行第三次BA优化位姿", Verbose::VERBOSITY_NORMAL);
                                // 第三次BA优化
                                nGood = Optimizer::PoseOptimization(&mCurrentFrame);
...
/**
 * @brief IMU初始化后更新了关键帧的位姿，因此还需要更新每一普通帧的位姿。LocalMapping中IMU初始化中调用
 * 1. 先用尺度更新 相对位姿中的平移部分
 * 最开始速度定义于IMU初始化时，每个关键帧都根据位移除以时间得到，经过非线性优化保存于KF中.
 * 之后使用本函数，让上一帧与当前帧 分别与他它们对应的上一关键帧做速度叠加得到，后面新的frame速度由上一个帧速度决定，如果使用匀速模型（大多数情况下），通过imu积分更新速度。
 * @param  s 更新后的 尺度
 * @param  b 更新后的 初始关键帧的 陀螺仪和加速度计零偏
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    ...
    // 若当前帧还未预积分，则等待其预积分完毕 (这段函数是在LocalMapping里调用的)
    ...
    // 上一帧 是 上一帧的上一关键帧，则设置上一帧的IMU位姿和速度 = 上一帧的上一关键帧的IMU旋转、位置、速度
    ...
    // 不是，则重新计算 上一帧的IMU旋转、位置、速度 (根据上一帧的上一个关键帧的IMU信息 和 上一帧的IMU预积分结果)
    else
    {
    ...
    // 当前帧已完成预积分，则重新计算 当前帧的IMU旋转、位置、速度 (根据当前帧的上一关键帧的IMU信息 与 当前帧的预积分结果)
...
void Tracking::SaveSearchByBowFailed(Frame &F, KeyFrame* &KF, std::vector<MapPoint*> &vMapPointMatches, const int img_idx)
{
    ...
    {
        if(vMapPointMatches[i])    // 存在地图点
        {
            cv::DMatch m{i, 0, 256};
            tuple<int, int> indexes = vMapPointMatches[i]->GetObservations()[KF]; // 该参考关键帧地图点 被其中哪个特征点观测到
    ...
    if(img_idx == 3)
    {
        pathMatches = "./Images/Relocalization/SBB_Match_F_" + std::to_string(F.mnId) + "_KF_" + std::to_string(KF->mnFrameId) + "_fp_" + std::to_string(F.mvKeysUn.size()) + "_kfp_" + std::to_string(KF->mvKeysUn.size()) + "_matches_" + std::to_string(matches.size()) + "_" + std::to_string(img_idx) + ".png";
    }
    else
        pathMatches = "./Images/SBB_Match_F_" + std::to_string(F.mnId) + "_KF_" + std::to_string(KF->mnFrameId) + "_fp_" + std::to_string(F.mvKeysUn.size()) + "_kfp_" + std::to_string(KF->mvKeysUn.size()) + "_matches_" + std::to_string(matches.size()) + "_" + std::to_string(img_idx) + ".png";
