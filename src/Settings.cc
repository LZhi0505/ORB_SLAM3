...
void Settings::readCamera1(cv::FileStorage &fSettings) {
    ...
    if (cameraModel == "PinHole") {
        ...
        // 单目/单目+IMU，且有畸变参数，则需对特征点进行畸变矫正。PinHole双目、KB鱼眼、Rectified不做特征点去畸变
...
// 获取右目相机的内参、畸变参数。如果是双目会被调用
void Settings::readCamera2(cv::FileStorage &fSettings) {
    bool found;
    vector<float> vCalibration;
    // 相机类型为PinHole，则读取的内参、畸变参数，且设置双目校正标志位为true
    ...
    // 若相机模型为Rectified，不进行双目极线矫正，只读取 基线b、bfx
    if (cameraType_ == Rectified) {
        b_ = readParameter<float>(fSettings, "Stereo.b", found);
        bf_ = b_ * calibration1_->getParameter(0);
    }
    // 若相机模型为Pinhole、KannalaBrandt，则读取双目矫正参数：右目到左目的变换矩阵
    // (其中t表示右目在左目坐标系下的坐标)、基线(平移向量的模长)、bfx
    else {
